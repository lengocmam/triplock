...
export default function AdminBookingsPage() {
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, `bao-cao-dat-ve-${Date.now()}.xlsx`);
  };

  useEffect(() => {
    apiClient.get('/admin/bookings').then((res) => setBookings(res.data)).finally(() => setLoading(false));
  }, []);

  return (
    <div>
      {/* ĐÃ SỬA: thêm div bọc tiêu đề + nút Xuất Excel nằm ngang hàng */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2>Đơn đặt vé gần đây</h2>
        <button className="btn btn-secondary" onClick={exportExcel}>📥 Xuất Excel</button>
      </div>
